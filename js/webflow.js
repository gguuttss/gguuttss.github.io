...
let stabIdAddress = "resource_tdx_2_1nfr5a7dsq78yst2dl5sr5e97uk3ujh5sx6xafd0ga0mdt6377cq5l0"
let componentAddress = "component_tdx_2_1crdhwrrlecfy6dm7tjnufruuzsdk8zj67xcc5pstxfkr3wavk3vqfl"
let poolComponentAddress = "component_tdx_2_1cp56lc9tat4jjt27nhd8p6pzt0uf9w94er2u6jefpnuds0evf59e6d"
let poolAddress = "pool_tdx_2_1ck6qxuffepuuklfytj6wze0sth6559u6jlgtkua2pl68u9n62g8wze"
let stabAddress = "resource_tdx_2_1t5tuj0zegpe2qvhzd4waqrhc2a8mclhcuzjseyvj3cun0zrzejfhct"
let cdpAddress = "resource_tdx_2_1ntvl7fq2s24pg7g3qvysrzjkal68syf2lkepnfwj99zsexw7vn0qyh"
let xrdAddress = "resource_tdx_2_1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxtfd2jc"
let ilisAddress = "resource_tdx_2_1t4cx4dl4f28luj5hywgumvu4wuev5cswft2d3phj90csz4ldceuwtd"
let lpAddress = "resource_tdx_2_1thpyhj6u6gxfaq0agp0qwrrjakcqcnh7cfr4a40m6qduq04zrf2890"
let liqAddress = "resource_tdx_2_1ntle3cyj38ukw8jfhmmzx06jlyhfta0nyq27c6aaj7t9ukpvxs6tnq"
let markerAddress = "resource_tdx_2_1ngp4rruzumnqn7wwthzl0w5lgu4uvqwc73etgd48jd3ua86dqu0c27"
let stakingAddress = "component_tdx_2_1cq0y2h68run78v5yef2qdu2nnpjsdfzm5pfzdmcsu3tvxsc52yqepe"
let accountAddress = "account_tdx_2_129kt8327ulqyq0ahdh74plu0r23qn9jugxppehggtp27m9n063heec"
let stakesKvs = "internal_keyvaluestore_tdx_2_1krm2ss074qlsnwygn4lyvks9p88k0zajyvxk804qvd3h83yajj3gx5"
let kvsHexes = ["5c805dc24bcb5cd20c9e81fd405e070c72edb00c4efec2475ed5fbd01bc03ea2", "5c805d706ab7f54a8ffe4a972391cdb3957732ca620e4ad4d886f22bf10157ed"]
let selectedText;
let xrdPrice;
let interestRate;
let stab_ids = [];
...
async function update_id() {
    if (window.location.pathname === '/incentives') {
        ...
        if (document.getElementById('id-select').value == "") {
            return;
        }
    ...
    if (window.location.pathname === '/liquidations') {
        (async () => { // Wrap your code in an async function
            let request2 = {
                ...
                "non_fungible_ids": [document.getElementById("marker-select").value],
            };
...
function update() {
    ...
    rdt.walletApi.walletData$.subscribe((walletData) => {
        ...
        function useData(data) {
            ...
            cdp_ids = []
            marker_ids = []
            internalPrice = data[1].details.state.fields[13].value;
            xrdPrice = data[1].details.state.fields[21].value;
            ...
            currentPeriod = data[3].details.state.fields[2].value;
            ...
            xrdPoolAmount = getResourceAmount(xrdAddress, data, 5);
            stabPoolAmount = getResourceAmount(stabAddress, data, 5);
            ...
            if (window.location.pathname === '/liquidations') {
                ...
                if (document.getElementById('marker-select').value != "") {
                    update_liq();
                }
            }
            ...
            if (window.location.pathname === '/manage-loans') {
                ...
                if (document.getElementById('cdp-select').value != "") {
                    update_cdp();
                }
            }
        }
    ...
    document.getElementById('sell').onclick = async function () {
        ...
        fetch('https://stokenet.radixdlt.com/state/entity/details', {
            ...
            .then(data => {
                ...
                document.getElementById('market-price-usd').innerHTML = "$" + ((xrdPoolAmount / stabPoolAmount) * xrdPrice).toFixed(2);
                document.getElementById('interest-rate').innerHTML = interestRate + "%/year";
                if (xrdPoolAmount / stabPoolAmount < 0.99 * stabXrdRatio) {
                    document.getElementById("recommendation").innerHTML = `<strong>Market price &lt; Internal price</strong>, sell XRD to arbitrage!`;
                } else if (xrdPoolAmount / stabPoolAmount > 1.01 * stabXrdRatio) {
                    document.getElementById("recommendation").innerHTML = `<strong>Market price &gt; Internal price</strong>, sell STAB to arbitrage!`;
                }
                else {
                    document.getElementById("recommendation").innerHTML = `Price error is within acceptable bounds. Minimal arbitrage opportunities.`;
...
if (window.location.pathname === '/liquidations') {
    document.getElementById('marker-select').onchange = async function () {
        update_liq();
    }
    // *********** mark ***********
    ...
    "burn_non_fungibles"
    Address("${cdpAddress}")
    Array<NonFungibleLocalId>(NonFungibleLocalId("${markerId}"));
      `
...
if (window.location.pathname === '/borrow') {
    ...
    document.getElementById("collateral-select").onchange = async function () {
        ...
        let validatorAddress = getValidatorAddress(address);
